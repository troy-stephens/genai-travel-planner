...
namespace api_roadtrip_test.Util
{
    ...
    public class LocalSearchService
    {
        ...
        public async Task<Dictionary<string, PointOfInterest>> getSearchData(Dictionary<string, PointOfInterest> pointsOfInterest)
        {
            ...
            foreach (var kvp in pointsOfInterest)
            {
                ...
                if (kvp.Value != null)
                {
                    ...
                    tasks.Add(SendRequestAsync(_localSearchSettings.Endpoint, _localSearchSettings.ApiKey, latitude, longitude, name, id));
                }
            }
            ...
            {
                Console.WriteLine("Mismatch between Input and LocalSearch data count.");
            ...
            foreach (var kvp in pointsOfInterest)
            {
                ...
                {
                    if (kvp.Key == result.Item1)
                    {
                        JObject? jsonObject = result.Item2 as JObject;
                        ...
                        if (jsonObject != null)
                        {
                            ...
                            if (placesArray != null && placesArray.Count > 0)
                            {
                                ...
                                if (place != null)
                                {
                                    ...
                                    JObject? jAddress = place["location"]?["address"] as JObject;
                                    Address address = jAddress.ToObject<Address>();
        ...
        private static async Task<(string, JObject?)> SendRequestAsync(string apiUrl, string accessToken, double lat, double lon, string name, string id)
        {
